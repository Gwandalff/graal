...
public class WasmGE extends WasmNode {
	...
	public WasmGE(WasmModule wasmModule, WasmCodeEntry codeEntry, int byteLength, byte type, boolean signed) {
		super(wasmModule, codeEntry, byteLength);
		this.type = type;
		this.signed = signed;
	}
	
	public WasmGE(WasmModule wasmModule, WasmCodeEntry codeEntry, int byteLength, byte type) {
		super(wasmModule, codeEntry, byteLength);
		this.type = type;
		this.signed = true;
	}
	...
	public TargetOffset execute(WasmContext context, VirtualFrame frame) {
		...
		case ValueTypes.I32_TYPE: {
            ...
            boolean result = signed ? y >= x : Integer.compareUnsigned(y, x) >= 0;
            pushInt(frame, context.stackpointer, result ? 1 : 0);
            context.stackpointer++;
            trace("0x%016X >=" + (signed?"s":"u") + " 0x%016X ? [i32]", y, x);
			break;
		}
		case ValueTypes.I64_TYPE: {
            ...
            boolean result = signed ? y >= x : Long.compareUnsigned(y, x) >= 0;
            pushInt(frame, context.stackpointer, result ? 1 : 0);
            context.stackpointer++;
            trace("0x%016X >=" + (signed?"s":"u") + " 0x%016X ? [i64]", y, x);
			break;
		}
		...
		default:
			throw new WasmTrap(this, "GE cannot have the void type.");
		}
		return null;
	}
